...
{  
  ...
  if (millis() - this->last_step_time >= this->step_delay) {
      ...
      stepMotor(this->step_number % 4);//'%' is the modulo operator, yields 1,2,3,4 depending how 'close'
      //steps_left is to a multiple of 4.
    }
  }
}
...
void Stepper::stepMotor(int thisStep)
{
  ...
  if (this->pin_count == 4) {
    switch (thisStep) {
      case 0:    // 1000
      digitalWrite(motor_pin_1, HIGH);//these are modified for the Arduino Kit geared stepper motor
      digitalWrite(motor_pin_2, LOW);
